#!/usr/bin/env python3
# Fast sampler of the drive state (chip1 factory, SWD non-halt).
# Catches the idle→drive TRANSITION: when the factory creates the A4963 drivers, enables SPI1
# and brings the motor path to life. Single SWD session, ~1 sample/sec, each line with t+state.
#
#   ./p1_drive_sampler.py 90 > /tmp/p1_drive_loop.txt 2>&1     # sample for 90 sec
#
# Reads only (fast): state, driver pointers, live reg7 of the left, SPI1
# CTL0/STAT/DATA(diag), TIMER2 CH2/CH3 duty, RCU APB1EN (SPI1EN bit14), GPIOD OCTL.
# NO halt. Look at lines where state!=3 (4=departure/5=cutting/6=findborder).
import sys, time
from swd import SWD
...
def main():
    try:
        sys.stdout.reconfigure(line_buffering=True)   # flush each line immediately (survives kill)
    except Exception:
        pass
    S = SWD()
    ...
    while time.time() - t0 < DUR:
        ...
        god  = w(S, 0x4001140c)                 # GPIOD OCTL (CS bits3/4/5)
        ...
        if lp and (lp >> 24) == 0x20:           # left driver created → live reg7
            r = w(S, lp + 0x3e - 2, 1)          # word containing reg7 (halfword @+0x3e)
            if r:
                lreg7 = (r[0] >> 16) & 0xFFFF
