...
: > "$LOG"
echo "START integrity $(date -u +%H:%M:%S) (ДЕРЖИ кнопку)" >> "$LOG"
...
for i in $(seq 1 200); do
  ...
  if echo "$OUT" | grep -q "HALTED, проверяю"; then
    echo "--- caught attempt $i ---" >> "$LOG"
    echo "$OUT" | grep -E "HALTED|MISMATCH|READ ERR|ЦЕЛОСТНОСТЬ|НАЙДЕНО|done" >> "$LOG"
    break
  fi
  sleep 0.2
done
