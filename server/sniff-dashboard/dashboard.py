#!/usr/bin/env python3
# Sniffer dashboard ON THE SERVER. Receives the stream from uart-sniffer (:9000),
# shows ESP status (RSSI/mode/errors) + live decoded frames (:8080).
# stdlib only.
...
def reader(conn):
    sess = time.strftime("session-%Y%m%d-%H%M%S")   # new session on every ESP connection
    ST["connected"] = True; ST["session"] = sess; ST["a"] = 0; ST["b"] = 0
    FA.clear(); FB.clear()
    f = open(OUT + "/" + sess + ".log", "ab", buffering=0)
    print("new session:", sess, flush=True)
...
<h2>&#128225; mower-sniff &mdash; status (server)</h2>
<div style="color:#7a869a;margin-bottom:8px">Session: <b id=sess>&mdash;</b> &middot; ESP buffer: <span id=buf>0</span> B &middot; lost: <span id=drop>0</span> B</div>
<div class=row>
 ...
 <div class=card><div class=k>Mode</div><div class=v>Sniffer @<span id=bd>&mdash;</span></div></div>
 <div class=card><div class=k>Errors</div><div class=v id=err>&mdash;</div></div>
</div>
<div class=row style=margin-top:10px>
 <div class=card><div class=k>Channel &#8593; (A, GPIO17)</div><div class=v id=ca>0</div></div>
 <div class=card><div class=k>Channel &#8595; (B, GPIO16)</div><div class=v id=cb>0</div></div>
</div>
<div class=row style=margin-top:10px>
 <div class=card style=flex:1><div class=k>&#8593; A (decoded)</div><pre id=fa></pre></div>
 <div class=card style=flex:1><div class=k>&#8595; B (decoded)</div><pre id=fb></pre></div>
</div>
<script>
async function t(){try{let s=await(await fetch('/state')).json();
 document.getElementById('esp').innerHTML=s.online?'<span class=ok>online</span>':'<span class=no>offline</span>';
 ...
 let e=[]; if(!s.online)e.push('ESP offline'); if(s.online&&s.a==0&&s.b==0)e.push('no data from mower');
 document.getElementById('err').innerHTML=e.length?'<span class=warn>'+e.join(', ')+'</span>':'<span class=ok>none</span>';
