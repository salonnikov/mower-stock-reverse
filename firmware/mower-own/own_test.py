#!/usr/bin/env python3
"""
own_test.py — control/check OUR OWN mower-own firmware over SWD-mailbox (poll.cfg :4444).
Run ON THE RASPBERRY PI. Wheels are JACKED UP, do NOT turn the blade on without need, mower on power.

Mailbox 0x20014000 (mower-own):
  +0x00 u32 heartbeat  (firmware++)   +0x04 u32 magic(0x5243414D)   +0x08 u32 seq(dead-man, we++)
  +0x0C i16 left_speed  +0x0E i16 right_speed  +0x10 u8 blade_on

Commands:
  ./own_test.py hb                 — heartbeat x5 (increments = our firmware alive)
  ./own_test.py state              — heartbeat/CH3CV(L)/CH2CV(R)/CH1CV(blade)
  ./own_test.py drive <L> <R>      — set speeds (signed, |v|<=0x1c20), arm, bump seq
  ./own_test.py fwd <spd>          — both forward
  ./own_test.py stop               — speeds 0, clear magic
  ./own_test.py blade <0|1>        — blade on/off (DELIBERATELY!)
  ./own_test.py hold               — keep seq alive ~10s (dead-man), so wheels don't stall
"""
import sys, time
from swd import SWD
...
def set_speeds(S,l,r):
    # left@0xC, right@0xE as i16 in one word 0xC
...
def main():
    ...
    if cmd=="hb":
        ...
        set_speeds(S,0,0); bump_seq(S); time.sleep(0.2); w(S,MB+4,0)  # speeds 0, clear magic
