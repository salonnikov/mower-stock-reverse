// coil-scope / passenger-scout — the ESP32 listens to the world, we watch from a phone.
//  • Coil (twisted pair) → GPIO36 (ADC1): wire signal — waveform, spectrum, frequency.
//  • IMU LSM6DS3 over I2C (SDA=21, SCL=22): tilt + angular rate (gyroscope).
//
// HOW TO VIEW: connect your phone to Wi-Fi "coil-scope" (password mower1234),
//              open http://192.168.4.1/
//
// IMU LSM6DS3: VIN→3V3, GND→GND, SCL→22, SDA→21, CS→3V3 (I2C mode). Nothing else needed.
// VERSION 3 (AP + IMU).
...
// Home Wi-Fi (opt.) — from secrets.h (not in git). Without it — only own AP.
...
// --- access point (always brought up, as a fallback channel) ---
const char* AP_SSID = "coil-scope";
const char* AP_PASS = "mower1234";     // min. 8 chars; "" = open network
String g_staIp = "—";                  // IP on the home network
...
uint8_t  imuAddr = 0;           // 0 = not found; otherwise 0x6A/0x6B
uint8_t  imuWho  = 0;           // WHO_AM_I
char     g_i2c[80] = "";        // what was found on the I2C bus
char     g_order[28] = "21/22"; // working SDA/SCL pin order
int      g_spiWho = -1;          // WHO_AM_I over SPI (-1 = not tried)

// --- SPI pins for chip probe (VSPI): SCK=18, MISO=19, MOSI=23, CS=5 ---
...
float    g_tilt=0;                     // angle from vertical, °
...
volatile double g_holdFreq = 0, g_holdMag = 0;   // peak hold
uint32_t g_holdT = 0;
uint16_t g_wave[256], g_spec[256];
// peak search window: ignore mains (<300 Hz) and the Nyquist artifact (>38 kHz)
...
int i2cScan() {
  ...
  if (n == 0) strcpy(g_i2c, "empty (nothing responds)");
  return n;
}
void imuSetup() {
  // try both orders: 21=SDA/22=SCL and vice versa (in case wires are swapped)
  Wire.end(); Wire.begin(21, 22); Wire.setClock(100000);
  int n = i2cScan();
  if (n == 0) {
    Wire.end(); Wire.begin(22, 21); Wire.setClock(100000);
    int n2 = i2cScan();
    if (n2 > 0) strcpy(g_order, "22/21 (wires swapped!)");
  }
  ...
  if (!imuAddr) return;
  imuWrite(0x10, 0x60);   // CTRL1_XL: accel 104 Hz, ±2g
  imuWrite(0x11, 0x60);   // CTRL2_G:  gyro 104 Hz, 245 dps
}
void imuPoll() {
  if (!imuAddr) return;
...
void analyzeBlock() {
  ...
  // find the peak manually in the window [F_MIN..F_MAX] — past 50 Hz mains and the 40 kHz artifact
  const double binHz = (double)SAMPLE_RATE / SAMPLES;     // 78.125 Hz/bin
  ...
  // peak hold ~3 s (so the real peak isn't lost in jitter)
...
<h3>passenger-scout</h3>
<div class=r>home net: <span id=ip>—</span> &nbsp; (or http://coil-scope.local/)</div>
<div>Wire — frequency: <b id=f>—</b> Hz &nbsp; amplitude: <span id=m>—</span></div>
<div class=r>max over 3s: <span id=hf>—</span> Hz (ampl <span id=hm>—</span>)</div>
<div>Waveform:</div><canvas id=w width=512 height=150></canvas>
<div>Spectrum (0…40 kHz):</div><canvas id=s width=512 height=150></canvas>
<h4>IMU <span id=imu class=r></span></h4>
<div class=r>I2C on bus: <span id=bus>—</span> &nbsp; order: <span id=ord>—</span></div>
<div class=r>SPI WHO_AM_I: <b id=spi>—</b> (expecting 0x69 = chip alive)</div>
<div>Tilt: <b id=t>—</b>° &nbsp; ang. rate Z: <span id=gz>—</span> °/s</div>
<div class=r>accel g: <span id=a>—</span> &nbsp; gyro °/s: <span id=g>—</span></div>
<script>
...
imu.textContent=d.imu?('('+d.imu+')'):'NOT FOUND';bus.textContent=d.bus;ord.textContent=d.ord;
...
void setup() {
  Serial.begin(115200);
  WiFi.mode(WIFI_AP_STA);                       // both own AP and home network
  ...
  if (strlen(STA_SSID)) {                        // connect to home Wi-Fi
    WiFi.begin(STA_SSID, STA_PASS);
    for (int i = 0; i < 40 && WiFi.status() != WL_CONNECTED; i++) delay(250);
    if (WiFi.status() == WL_CONNECTED) {
      g_staIp = WiFi.localIP().toString();
      Serial.print("Home net -> http://"); Serial.println(g_staIp);
    } else Serial.println("Home net: not connected (check name/password)");
  }
  ...
  imuSetup();
  if (imuAddr) Serial.printf("IMU LSM6DS3 at 0x%02X (WHO=0x%02X)\n", imuAddr, imuWho);
  else         Serial.println("IMU not found over I2C");
  spiCheck();                                  // chip probe over SPI
  Serial.printf("SPI WHO_AM_I = 0x%02X (expecting 0x69)\n", g_spiWho);
  adcSetup();
}
...
// ----- NOTES -----
// IMU "NOT FOUND": check CS→3V3 (without it the chip is in SPI), SDA=21/SCL=22, VIN→3V3.
// Weak wire signal: get closer to the wire/base, rotate the coil, add turns.
// Frequency out of range: say so — I'll rebuild with a different SAMPLE_RATE.
